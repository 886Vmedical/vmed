...
public class BlePeripheralSettings {
    ...
    int mtuSize = MTU_SIZE_DEFAULT;

    BlePeripheralSettings() {
    ...
    public void setParameter(@NonNull final Bundle bundle) {
        ...
        if (bundle.containsKey(Key.MtuSize.name())) {
            mtuSize = bundle.getInt(Key.MtuSize.name());
        }
    }
    ...
    public Bundle getParameter(@Nullable List<String> keys) {
        ...
        for (String key : keys) {
            ...
            if (Key.MtuSize.name().equals(key)) {
                bundle.putInt(Key.MtuSize.name(), mtuSize);
            }
