...
public class DiscoverPeripheral implements Parcelable {
   ...
   */
  public static final Creator<DiscoverPeripheral> CREATOR = new Creator<DiscoverPeripheral>() {
  ...
  private static final int TYPE_LOCAL_NAME = 0x09;
  private static final int TYPE_LOCAL_NAME_SHORT = 0x08;
  ...
  DiscoverPeripheral(@NonNull final BluetoothDevice bluetoothDevice, final int rssi,
  ...
  private List<BleAdvertiseData> parseAdvertiseData(byte[] payload) {
    ...
    for (int i = 0; i < payload.length; ) {
      ...
      if(type == TYPE_LOCAL_NAME || type == TYPE_LOCAL_NAME_SHORT){
        mDeviceName = new String(data).trim();
      }
  ...
  @NonNull public BluetoothDevice getBluetoothDevice() {
