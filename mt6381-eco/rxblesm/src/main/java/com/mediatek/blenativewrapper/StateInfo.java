...
public class StateInfo implements Parcelable {
  ...
  static {
    ...
    sStateMap.put(DetailedState.MtuRequesting, ConnectionState.Connecting);
    ...
    sStateMap.put(DetailedState.BluetoothOff, ConnectionState.Disconnected);
  }
  ...
  public String getText() {
    return String.format(
        "mBondState:%s mConnectionState:%s mDetailedState:%s mAclConnectionState:%s mGattConnectionState:%s",
        mBondState.name(), mConnectionState.name(), mDetailedState.name(), mAclConnectionState.name(), mGattConnectionState.name());
  }
  ...
  public void setConnectionState(ConnectionState connectionState, boolean isNotify) {
    synchronized (this) {
  ...
  public enum ConnectionState {
    Disconnected, Connecting, Connected, Disconnecting, Unknown
  }

  public enum DetailedState {
    Disconnected, Pairing, GattConnecting, ServiceDiscovering,MtuRequesting, Cleanup, CommunicationReady, Communicating, Disconnecting, Dead, BluetoothOff, Unknown
  }

  public enum Reason {
    ConnectRequest, BluetoothOff, AlreadyConnected, OSNativeError, DisconnectRequest, DestroyRequest,DidDisconnection, EncryptionFailed, Timeout, Unknown
  }

  public enum BondState {
    NotBonded, Bonding, Bonded, Unknown
  }
